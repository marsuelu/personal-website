...
- chrome调试工具/performance/LCP 显示有一处颜文字‘（_´▽｀_）’加载花费时间最长21.4s,改成一般文字后速度变成8.11s
...
## gsap scrollTrigger onUpdate参数self.scroll()数值不正确

-最开始以为是[https://gsap.com/community/forums/topic/31023-scrolltrigger-wrong-start-end-positions/]中提到的原因，后面发现window.scrollY获取到的数据就是错的